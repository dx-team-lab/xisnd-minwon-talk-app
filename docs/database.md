...
전환 규칙 요약
Firestore 컬렉션 1개 = MySQL 테이블 1개 (기본 원칙)
문서 내 배열 필드는 별도 테이블로 분리 (정규화)
Firebase 자동 생성 문자열 ID → `BIGINT AUTO_INCREMENT` PK로 교체
`Timestamp` → `DATETIME NOT NULL` 으로 교체
---
2. 현재 Firebase 컬렉션 목록 및 MySQL 테이블 전환 방향
Firebase 컬렉션	역할	MySQL 테이블 전환 방향
`caseExamples`	민원 사례 데이터	`tb_case_example` 테이블
`sites`	현장 정보	`tb_site` 테이블
`sites/{id}/complaints`	현장별 민원 (서브컬렉션)	`tb_site_complaint` 테이블 (FK: site_id)
`sites/{id}/siteImages`	현장 이미지 (서브컬렉션)	`tb_site_image` 테이블 (FK: site_id)
`users`	사용자 계정	`tb_user_master` 테이블
`roles_admin`	관리자 역할 목록	`tb_user_role` 테이블로 통합  (FK: user_id)
`roles_manager`	매니저 역할 목록	`tb_user_role` 테이블로 통합
`responseGuides`	대응 가이드	`tb_response_guide` 테이블
`actionPlanLinks`	대응 방안 링크	`tb_action_plan` 테이블 (FK: guide_id)
`settings`	시스템 설정	`tb_system_setting` 테이블
`references`	참고 자료	`tb_reference` 테이블
`activity_logs`	활동 로그	`tb_activity_log` 테이블
`inquiries`	문의 내용	`tb_inquiry` 테이블
`code`	코드 마스터	`tb_code_master` 테이블로 통합
---
3. 정규화 규칙
왜 정규화가 필요한가?
...
```json
{
  "type": ["소음", "진동"],         ← 배열 → tb_case_example_type 테이블로 분리
  "requestContent": ["보상", "조정"] ← 배열 → tb_case_example_request 테이블로 분리
}
```
`sites` 문서의 배열 필드
```json
{
  "phase": ["착공전", "토공"]  ← 배열 → tb_site_phase 테이블로 분리
}
```
정규화 판단 기준
상황	처리 방법
값이 2개 이상 선택 가능한 배열	별도 테이블로 분리
코드성 데이터 (유형, 지역, 단계 등)	`tb_code_master` 테이블로 통합 관리
...
PK 컬럼명은 `tb_` 접두사를 제외한 테이블명 + `_id` 형식으로 한다.
예: `tb_case_example` → `case_example_id`
예: `tb_site` → `site_id`
Firebase의 문자열 ID(`abc123xyz`)는 마이그레이션 시 정수 PK로 대체하며, 필요 시 `firebase_id VARCHAR(100)` 컬럼으로 별도 보관한다.
FK 규칙
FK 컬럼명은 참조 대상 PK명과 동일하게 작성한다.
예: `tb_site_complaint.site_id` → `tb_site.site_id` 참조
실제 DB FK 제약조건(`FOREIGN KEY`) 적용 여부는 개발자가 운영 정책에 따라 결정한다.
...
---
7. 테이블 네이밍 규칙
모든 테이블명은 `tb_` 접두사를 붙인다.
테이블명은 소문자 + 언더스코어(snake_case) 를 사용한다.
복수형 대신 단수형을 사용한다.
✅ `tb_case_example` / ❌ `caseExamples`, `case_examples`, `tb_caseExample`
연결 테이블(다대다 관계)은 두 테이블명을 조합한다.
예: `tb_case_example_type` (tb_case_example + type의 연결)
---
8. 코드성 데이터 관리 규칙
현재 프론트엔드 `constants.ts`에 하드코딩된 선택지들은 DB에서 관리한다.
현재 상수	항목 예시	DB 관리 방법
`FILTER_OPTIONS.region`	공업, 주거, 민감, 상업	`tb_code_master` 테이블
`FILTER_OPTIONS.phase`	착공전, 토공, 골조...	`tb_code_master` 테이블
`FILTER_OPTIONS.type`	소음, 진동, 교통...	`tb_code_master` 테이블
`PROGRESS_OPTIONS`	접수, 진행, 종결	`tb_code_master` 테이블
`COMPENSATION_STATUS_OPTIONS`	미보상, 과태료...	`tb_code_master` 테이블
`REQUEST_TYPE_OPTIONS`	정신적피해보상...	`tb_code_master` 테이블
코드 테이블 구조 (예시)
```
tb_code_master: 코드 그룹 관리
  code_master_id | code_group_name | description | code_value | code_name | sort_order

와 같이 tb_code_master 테이블에 코드를 통합한다.
```
---
9. 사용자 테이블(tb_user_master) 마이그레이션 주의사항
비밀번호 컬럼 설계
비밀번호 저장 컬럼명은 `password_hash`로 지정하여 해시값을 담는 컬럼임을 명확히 한다.
...
```sql
-- 예시: tb_user_master 테이블의 비밀번호 컬럼
...
인덱스 예상 대상 컬럼
테이블	인덱스 대상 컬럼	이유
`tb_case_example`	`region`, `phase`, `progress`	필터 검색 대상
`tb_case_example`	`created_at`	최신순 정렬
`tb_site_complaint`	`site_id`	FK 조회
`tb_activity_log`	`created_at`, `actor_email`	로그 조회
`tb_user_master`	`login_id`	로그인 조회
